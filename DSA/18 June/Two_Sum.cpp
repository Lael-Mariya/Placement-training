...
public:
    vector<int> twoSum(vector<int>& nums, int target) {
        ...
        for (int i=0; i<len; i++) {
            arr.push_back({nums[i],i});   // store the elements in pairs with their og indices, to retrieve the original indices of the unsorted array as the array is sorted for further operation. 
        }  
